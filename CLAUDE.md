...
- **Docker VM memory raised to 32 GB** (2026-06-19). Set via `"MemoryMiB": 32768` in `~/Library/Group Containers/group.com.docker/settings-store.json` (backup: `settings-store.json.bak-20260619`), then `docker desktop restart`. Verify: `docker info --format '{{.MemTotal}}'` → ~31.3 GB. **Why:** this host has 128 GB but Docker Desktop defaulted the VM to **7.75 GB**, starving the whole stack (the 359 GB Postgres' page cache *and* the worker). At 7.75 GB the worker hit .NET's GC heap hard limit (~5.8 GB) on a bounded ~7–9 GB per-cycle allocation and OOM-restarted every ~12 min — the **293-restart loop** (see the chunk-timeout note above; the restart loop was a *separate* issue from that timeout). ⚠️ **This is a host-level Docker Desktop setting, NOT in the repo or compose** — if Docker Desktop is reset/reinstalled it reverts to the 7.75 GB default and the restart loop returns. Re-apply via Settings → Resources → Memory (slider) or the `settings-store.json` key + a Docker restart.
